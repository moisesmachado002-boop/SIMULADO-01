(() => {
  ...
  async function ensurePreferences() {
    ...
    const created = await db.from('study_preferences').insert({ user_id:user.id, ...DEFAULTS }).select('*').single();
  ...
  async function savePreferences(patch) {
    ...
    const days = Array.isArray(patch.study_days)
      ? [...new Set(patch.study_days.map(Number))].filter(n => n >= 1 && n <= 7).sort()
      : old.study_days;
    const row = {
  ...
  async function setTodayMode(mode) {
    ...
    if (mode === 'normal') {
      ...
      let minutes = Number(prefs.daily_minutes || 0);
      if (mode === 'low_time') minutes = Math.min(minutes, Number(prefs.low_time_minutes || 20));
      if (mode === 'rest') minutes = 0;
      if (mode === 'extra') {
        const base = existing?.mode === 'extra' && Number.isFinite(Number(existing.minutes_override))
          ? Number(existing.minutes_override) : minutes;
        minutes = clamp(base + Number(prefs.extra_step_minutes || 30), 20, 720);
      }
      const saved = await db.from('study_day_overrides').upsert({
        user_id:user.id, study_date:today, mode, minutes_override:minutes, updated_at:new Date().toISOString()
  ...
  function reviewPriority(state, dueAt) {
    ...
    const overdueDays = Number.isFinite(due) ? Math.max(0, (Date.now() - due) / 86400000) : 0;
  ...
  async function regenerate(options = {}) {
    ...
    if (questionIds.length) {
      const stateResult = await db.from('user_question_state')
        ...
        .eq('user_id',user.id).in('question_id',questionIds);
    ...
    const states = new Map(stateRows.map(row => [row.question_id,row]));
    ...
    stateRows.forEach(state => {
      ...
      reviewCandidates.push({
        kind:'review', question_id:q.id, topic_id:q.topic_id, subject_id:q.subject_id,
        due:dueDate, priority:reviewPriority(state,state.next_review_at), duration:REVIEW_MINUTES,
        source_reason:dueDate <= today ? 'revisao_vencida' : 'revisao_programada', displaced_from:null
      });
    });
    ...
    questions.forEach(q => {
      if (!q.topic_id || Number(states.get(q.id)?.seen_count || 0) > 0) return;
      if (!newByTopic.has(q.topic_id)) newByTopic.set(q.topic_id,[]);
      newByTopic.get(q.topic_id).push(q);
    });

    fixed.filter(item => item.task_type === 'questions' && ['pending','in_progress'].includes(item.status)).forEach(item => {
      const list = newByTopic.get(item.topic_id);
      if (!list?.length) return;
      list.splice(0, Math.min(list.length, Number(item.question_target || 0)));
      if (!list.length) newByTopic.delete(item.topic_id);
    });
    ...
    const topicQueue = topics
      ...
      .map(topic => {
        ...
        return {
          topic, subject,
          score:Number(m?.mastery_score ?? 35),
          attempts:Number(m?.attempts_count || 0),
          last:m?.last_attempt_at ? new Date(m.last_attempt_at).getTime() : 0,
          newCount:newByTopic.get(topic.id).length
        };
      })
    ...
    const reviewPressure = reviewCandidates.filter(item => item.due <= today).length * REVIEW_MINUTES
      + debtCandidates.filter(item => item.kind === 'review').reduce((sum,item) => sum + item.duration,0);
    const inserts = [];
    let rotation = 0;
    ...
    for (let offset=0; offset<HORIZON_DAYS; offset+=1) {
      ...
      const eligibleReviews = reviewCandidates.filter(item => !item.assigned && item.due <= key)
      ...
      for (const item of eligibleReviews) {
        ...
        inserts.push({
          topic_id:item.topic_id, question_id:item.question_id, scheduled_for:key, task_type:'review',
          question_target:1, duration_minutes:item.duration, priority:item.priority,
          source_reason:item.source_reason, displaced_from:item.displaced_from,
          sort_order:order, status:'pending', progress_count:0
        });
        item.assigned = true;
      ...
      for (const item of debtEligible) {
        if (available < Math.min(REVIEW_MINUTES,item.duration)) break;
        const duration = Math.min(item.duration,available);
        inserts.push({
          topic_id:item.topic_id, question_id:item.question_id, scheduled_for:key,
          task_type:item.kind === 'review' ? 'review' : item.kind,
          question_target:item.question_target || (item.kind === 'review' ? 1 : Math.max(1,Math.floor(duration/QUESTION_MINUTES))),
          duration_minutes:duration, priority:item.priority, source_reason:item.source_reason,
          displaced_from:item.displaced_from, sort_order:order, status:'pending', progress_count:0
        });
        item.assigned = true;
        available -= duration;
        order += 10;
      ...
      while (available >= 10 && topicQueue.length && safety < topicQueue.length * 3) {
        ...
        inserts.push({
          topic_id:candidate.topic.id, question_id:null, scheduled_for:key, task_type:'questions',
          question_target:target, duration_minutes:duration,
          priority:clamp(75-Math.round(candidate.score/4),35,80),
    ...
    const deferIds = [...new Set(reviewCandidates.filter(item => !item.assigned && item.due <= horizon).map(item => item.question_id).filter(Boolean))];
    const publish = await db.rpc('publish_study_plan_atomic', {
      p_plan_version: PLAN_VERSION,
      p_today: today,
      p_rebalance_today: !!options.rebalanceToday,
      p_items: inserts,
      p_defer_question_ids: deferIds
    });
    if (publish.error || !publish.data?.ok) throw publish.error || new Error('Não foi possível publicar o novo cronograma.');
  ...
  async function getPlan() {
    ...
    for (const result of [itemsResult,overrideResult,topicsResult,subjectsResult]) if (result.error) throw result.error;
  ...
  async function completeItem(itemId) {
    const { db, user } = await userContext();
    if (!db || !user) return null;
    const current = await db.from('study_plan_items').select('task_type,question_target').eq('id',itemId).eq('user_id',user.id).maybeSingle();
    if (current.error) throw current.error;
    if (!current.data) throw new Error('Tarefa não encontrada.');
    if (['questions','review'].includes(current.data.task_type)) {
      throw new Error('Questões e revisões são concluídas automaticamente conforme você responde.');
    }
    const result = await db.from('study_plan_items').update({
      status:'completed', completed_at:new Date().toISOString(), progress_count:Number(current.data.question_target || 0)
  ...
  async function progressFromAttempt(questionId, topicId) {
    ...
    if (!db || !user) return null;
    const today = dateKey();
    const result = await db.from('study_plan_items').select('*')
      .eq('user_id',user.id).eq('plan_version',PLAN_VERSION).eq('scheduled_for',today)
    ...
    const update = await db.from('study_plan_items').update({
      progress_count:nextProgress, status:done ? 'completed' : 'in_progress', completed_at:done ? new Date().toISOString() : null
  ...
  window.addEventListener('mentor:attempt-saved', event => {
    if (event.detail?.planProgressHandled) {
      setTimeout(() => regenerate().catch(error => console.warn('P6: projeção não recalculada após tentativa atômica',error)),120);
      return;
    }
    progressFromAttempt(event.detail?.questionId,event.detail?.topicId)
      .then(() => regenerate().catch(error => console.warn('P6: projeção não recalculada',error)))
      .catch(error => console.warn('P6: progresso da missão não atualizado',error));
  });
