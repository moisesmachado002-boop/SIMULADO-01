(() => {
  'use strict';
  if(window.__mentorTimerQuestionsV413)return;
  window.__mentorTimerQuestionsV413=true;
  ...
  let current=null;
  function toast(text,kind='neutral'){const n=$('#toast');if(!n)return;n.textContent=text;n.dataset.kind=kind;n.classList.add('show');clearTimeout(window.__tqV413Toast);window.__tqV413Toast=setTimeout(()=>n.classList.remove('show'),3600);}
  function inject(){
    ...
    const active=$('#studyTimerActiveBox');if(active&&!$('#studyTimerQuestionStats')){
      ...
      const topic=active.querySelector('.st-topic');topic?.insertAdjacentElement('afterend',box);box.addEventListener('input',renderSummary);
    }
  }
  function elapsed(){if(!current)return 0;let s=Number(current.accumulated_seconds||0);if(current.status==='running'&&current.running_since){const x=Date.parse(current.running_since);if(Number.isFinite(x))s+=Math.max(0,Math.floor((Date.now()-x)/1000));}return s;}
  ...
  document.addEventListener('click',async e=>{
    ...
    try{
      ...
      const correct=Number(data?.correct_answers??q-errors),mins=Number(data?.duration_minutes||0),plan=data?.plan||{};
      const progress=plan?.matched?` • missão ${plan.progress}/${plan.target}`:'';
      toast(`${q} questões registradas: ${correct} acertos e ${errors} erros em ${mins} min${progress}.`,'ok');
  ...
  const obs=new MutationObserver(()=>{clearTimeout(window.__tqV413Mut);window.__tqV413Mut=setTimeout(()=>{inject();renderSummary();},80);});obs.observe(document.documentElement,{subtree:true,childList:true});
  setInterval(()=>{if(current?.activity_type==='questions')renderSummary();},1000);setTimeout(sync,600);
})();