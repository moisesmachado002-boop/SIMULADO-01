(() => {
  ...
  function toast(text,kind='neutral'){
    ...
    clearTimeout(window.__mentorPlanUiToast);window.__mentorPlanUiToast=setTimeout(()=>n.classList.remove('show'),3600);
  }
  ...
  const defaultWeek=()=>{const t=dateKey(),d=new Date(`${t}T15:00:00Z`),w=d.getUTCDay()||7;return w===7?addDays(t,1):mondayOf(t);};
  ...
  function injectStyle(){
    ...
    const s=document.createElement('style');s.id='mentorPlanUiV500Style';s.textContent=`
      #dailyTasks.v500-native-hidden{display:none!important}.v500-status{margin:0 0 12px;padding:11px 14px;border:1px solid #d8e7da;background:#f3faf4;border-radius:10px;display:flex;justify-content:space-between;gap:12px;align-items:center}.v500-status strong{font-size:12px}.v500-status span{font-size:10px;color:#65716a}.v500-status b{font-size:9px;background:#dff2e4;color:#1d7138;border-radius:999px;padding:5px 8px}.v500-status.warn{background:#fff6e5;border-color:#f1d39a}.v500-status.warn b{background:#ffe2a6;color:#734d00}
  ...
  async function context(start,end){
    ...
    const [planR,topicsR,subjectsR,policiesR,prefsR]=await Promise.all([
      c.from('study_plan_items').select('id,topic_id,subtopic_id,scheduled_for,task_type,question_target,progress_count,duration_minutes,status,sort_order,source_reason,completed_at').eq('user_id',user.id).gte('scheduled_for',start).lte('scheduled_for',end).neq('status','skipped').order('scheduled_for').order('sort_order'),
  ...
  function grouped(items,ctx){
    ...
    for(const x of items){
      ...
      if(!m.has(key))m.set(key,{key,topic_id:x.topic_id,subject_id:sid,review,policy:p,items:[]});m.get(key).items.push(x);
    }
  ...
  function taskAction(x){
    ...
    if(x.task_type==='questions'){const qc=/qconcursos|group_v428|feasible_v425|smart_week_v431_group/.test(x.source_reason||'');return `<button class="primary-button" ${qc?`data-task-qc="${x.id}"`:`data-task-bank="${x.id}"`}>Fazer questões</button><button class="secondary-button" data-task-complete="${x.id}">Concluir</button>`;}
  ...
  function dailyCard(g,ctx){
    ...
    const parent=g.review?`${items.length} assunto(s) vencido(s), agrupados por matéria.`:(st?`Parte de: ${t?.title||''}`:g.policy?.study_mode==='group'?'Bloco conjunto do mesmo assunto.':'');
  ...
  async function renderDaily(){
    const day=dateKey(),ctx=await context(day,day),native=$('#dailyTasks');if(!native)return;native.classList.add('v500-native-hidden');let host=$('#v500Daily');if(!host){host=document.createElement('section');host.id='v500Daily';native.parentElement?.insertBefore(host,native);}const items=ctx.plan,groups=grouped(items,ctx),mins=items.reduce((s,x)=>s+Number(x.duration_minutes||0),0),budget=Math.max(20,Math.floor(Number(ctx.prefs.daily_minutes||120)*(100-clamp(Number(ctx.prefs.buffer_percent||0),0,40))/100)),subjects=new Set(items.map(x=>ctx.topics.get(x.topic_id)?.subject_id).filter(Boolean)),doneMinutes=items.reduce((s,x)=>s+(x.status==='completed'?Number(x.duration_minutes||0):x.task_type==='questions'&&x.status==='in_progress'?Number(x.duration_minutes||0)*Math.min(1,Number(x.progress_count||0)/Math.max(1,Number(x.question_target||1))):0),0),rate=mins?Math.round(doneMinutes/mins*100):0,started=items.some(x=>['completed','in_progress'].includes(x.status)),over=mins>budget;
    host.innerHTML=`<div class="v500-status ${over?'warn':''}"><div><strong>${started?'Meta de hoje em execução':'Meta de hoje pronta'}</strong><br><span>${groups.some(g=>g.review)?'Revisões da mesma matéria ficam juntas. ':''}Configuração: ${ctx.prefs.daily_minutes||120} min/dia, ${ctx.prefs.review_ratio??40}% revisão, teto útil ${budget} min.</span></div><b>${over?'REVER CARGA':'ESTÁVEL'}</b></div><div class="v500-list">${groups.length?groups.map(g=>dailyCard(g,ctx)).join(''):'<div class="empty-state panel">Sem meta planejada para hoje.</div>'}</div>`;
    if($('#dailyDate'))$('#dailyDate').textContent=br(day).slice(0,5);if($('#dailySubjects'))$('#dailySubjects').textContent=subjects.size;if($('#dailyMinutes'))$('#dailyMinutes').textContent=`${mins} min`;if($('#dailyCompleted'))$('#dailyCompleted').textContent=`${rate}%`;if($('#dailyProgressText'))$('#dailyProgressText').textContent=`${rate}%`;if($('#dailyProgressBar'))$('#dailyProgressBar').style.width=`${rate}%`;
  }
  ...
  async function renderWeek(start=activeWeek||defaultWeek()){
    const ctx=await context(start,addDays(start,6)),host=$('#v423WeekGrid')||$('#weekBoard');if(!host)return;activeWeek=start;const studyDays=new Set((ctx.prefs.study_days||[1,2,3,4,5,6]).map(Number)),days=Array.from({length:7},(_,i)=>addDays(start,i));
    host.innerHTML=`<div class="v500-week-note"><div><strong>Planejamento centralizado</strong><br><span>Abrir a semana não altera nada. Salvar horários ou clicar em replanejar usa a mesma rotina do servidor.</span></div><div class="v500-week-actions"><b>SOMENTE LEITURA</b><button class="v500-replan" id="v500Replan">Replanejar esta semana</button></div></div><div class="v500-grid">${days.map(day=>{const items=ctx.plan.filter(x=>x.scheduled_for===day),groups=grouped(items,ctx),dow=new Date(`${day}T15:00:00Z`).getUTCDay()||7,total=items.reduce((z,x)=>z+Number(x.duration_minutes||0),0);return `<section class="v500-day"><div class="v500-day-head">${esc(weekday(day))}<br>${br(day)}<small>${groups.length?`${groups.length} bloco(s) • ${total} min`:studyDays.has(dow)?'sem foco registrado':'descanso'}</small></div><div class="v500-day-body">${groups.length?groups.map(g=>weekFocus(g,ctx)).join(''):(studyDays.has(dow)?'<div class="v500-empty">Sem meta planejada.</div>':'<div class="v500-rest">Descanso</div>')}</div>${groups.length?`<div class="v500-day-total">Total do dia: <strong>${total} min</strong></div>`:''}</section>`;}).join('')}</div>`;
  }

  async function savePreferences(btn){
    if(btn?.dataset.busy==='1')return;const daily=Number($('#prefDailyMinutes')?.value),review=Number($('#prefReviewRatio')?.value),buffer=Number($('#prefBuffer')?.value),days=$$('#weekdayPicker input:checked').map(x=>Number(x.value));
    if(!Number.isFinite(daily)||daily<20||daily>480||!days.length)throw new Error('Confira minutos e dias de estudo.');if(btn){btn.dataset.busy='1';btn.disabled=true;btn.textContent='Salvando...';}
    try{const c=db(),r=await c.rpc('update_study_preferences_v434',{p_daily_minutes:Math.round(daily),p_study_days:days,p_review_ratio:clamp(Math.round(Number.isFinite(review)?review:40),0,100),p_buffer_percent:clamp(Math.round(Number.isFinite(buffer)?buffer:15),0,40),p_timezone:TZ});if(r.error)throw r.error;if(!r.data?.ok)throw new Error('O servidor não confirmou a alteração.');toast('Configuração salva e cronograma recalculado.','ok');emit('preferences_replan');setTimeout(()=>location.reload(),550);}finally{if(btn){delete btn.dataset.busy;btn.disabled=false;btn.textContent='Salvar horários';}}
  }

  async function replan(start=activeWeek||defaultWeek(),confirmUser=true){
    if(busy)return;if(confirmUser&&!confirm('Replanejar esta semana? As revisões vencidas terão prioridade e as metas pendentes podem mudar de dia.'))return;busy=true;
    try{const r=await db().rpc('rebuild_smart_week_v431',{p_week_start:start});if(r.error)throw r.error;toast(r.data?.status==='frozen'?'Esta semana já começou e foi preservada.':'Semana replanejada.','ok');emit('stable_replan');await Promise.allSettled([renderDaily(),renderWeek(start)]);}finally{busy=false;}
  }

  function ensureSkipButton(){
    const card=$('#questionCard');if(!card)return;if($('#questionSkipButton'))return;const btn=document.createElement('button');btn.id='questionSkipButton';btn.type='button';btn.className='secondary-button';btn.textContent='Pular questão →';const confirm=$('#questionConfirmButton');if(confirm?.parentElement)confirm.parentElement.insertBefore(btn,confirm.nextSibling);btn.addEventListener('click',()=>{const fb=$('#questionFeedback');if(fb&&!fb.classList.contains('hidden'))return;$('#questionNextButton')?.click();toast('Questão pulada. Não contou como erro nem como progresso.');});
  }

  function boot(){
    injectStyle();setTimeout(()=>{renderDaily().catch(e=>console.warn('daily v500',e));renderWeek().catch(e=>console.warn('week v500',e));ensureSkipButton();},1200);
    document.addEventListener('click',e=>{
      ...
      const study=e.target.closest('[data-v500-study]');if(study){e.preventDefault();e.stopImmediatePropagation();$('#studyTimerPill')?.click();toast('Cronômetro iniciado para a meta de estudo.','ok');return;}
      if(e.target.closest('#dailyRefreshButton'))setTimeout(()=>renderDaily().catch(()=>{}),120);
      if(e.target.closest('#v423PrevWeek,#v423NextWeek,#v423CurrentWeek'))setTimeout(()=>{activeWeek='';renderWeek(defaultWeek()).catch(()=>{});},220);
    },true);
