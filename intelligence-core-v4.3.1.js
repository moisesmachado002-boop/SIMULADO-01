(() => {
  ...
  function indexes(m){
    ...
    for(const r of m.reviews){
      const topicId=r.topic_id||qById.get(r.question_id)?.topic_id;
      if(!topicId)continue;
      if(!reviewsByTopic.has(topicId))reviewsByTopic.set(topicId,[]);
      reviewsByTopic.get(topicId).push(r);
    }

    const dueReviews=(topicId,day)=>(reviewsByTopic.get(topicId)||[])
      .filter(r=>dateKey(new Date(r.due_at))<=day)
      .sort((a,b)=>forgettingRisk(stateByQ.get(b.question_id))-forgettingRisk(stateByQ.get(a.question_id))||Date.parse(a.due_at)-Date.parse(b.due_at));
    ...
    const topicScore=(topic,day)=>{
      ...
      const due=dueReviews(topic.id,day);
      const risk=(statesByTopic.get(topic.id)||[]).reduce((mx,st)=>Math.max(mx,forgettingRisk(st)),0);
      const weak=evidence>=TARGET?Math.max(0,80-accuracy)*2:Math.max(0,TARGET-evidence)*7;
      return due.length*55+risk+weak+(master.trend==='down'?25:0)+Math.min(15,unseen.get(topic.id)||0);
    };

    const focusTopic=(subjectId,day)=>m.topics
      .filter(t=>t.subject_id===subjectId)
      .sort((a,b)=>topicScore(b,day)-topicScore(a,day)||Number(a.position||0)-Number(b.position||0))[0]||null;

    return{subjectByName,masteryByTopic,stateByQ,available,unseen,reviewsByTopic,dueReviews,topicScore,focusTopic};
  }
  ...
  async function cleanupLegacyPending(m){
    const old=m.plan.filter(p=>p.status==='pending'&&(
      ...
      (p.task_type==='questions'&&(p.source_reason==='cap_fill_qconcursos'||String(p.source_reason||'').startsWith('cycle_v4_')||String(p.source_reason||'').startsWith('cycle_v41_')||String(p.source_reason||'').startsWith('cycle_v42_')))
    ));
  ...
  function desiredPlan(m,idx,dates){
    ...
    for(const day of dates){
      ...
      subjects.forEach((subject,slot)=>{
        const topic=idx.focusTopic(subject.id,day);if(!topic)return;
        const due=idx.dueReviews(topic.id,day);
        const reviewTarget=Math.min(3,due.length);
        const reviewMinutes=reviewTarget*QMIN;
        const questionMinutes=Math.max(QMIN,perSubject-reviewMinutes);
        const questionTarget=Math.max(4,Math.floor(questionMinutes/QMIN));

        if(reviewTarget){
          const first=due[0],risk=forgettingRisk(idx.stateByQ.get(first.question_id));
          desired.push({key:`${day}|review|${topic.id}`,scheduled_for:day,topic_id:topic.id,question_id:first.question_id,task_type:'review',question_target:reviewTarget,duration_minutes:reviewMinutes,priority:risk>=95?100:risk>=85?96:90,status:'pending',source_reason:`${SOURCE_PREFIX}review`,plan_version:PLAN,sort_order:slot*100+10,progress_count:0});
        }

        const useBank=(idx.unseen.get(topic.id)||0)>=questionTarget;
        desired.push({key:`${day}|questions|${topic.id}`,scheduled_for:day,topic_id:topic.id,question_id:null,task_type:'questions',question_target:questionTarget,duration_minutes:questionTarget*QMIN,priority:75,status:'pending',source_reason:useBank?`${SOURCE_PREFIX}bank`:`${SOURCE_PREFIX}qconcursos`,plan_version:PLAN,sort_order:slot*100+20,progress_count:0});
      });
    }
    return{desired,meta};
  }
  ...
  async function reconcileGenerated(m,desired){
    ...
    const desiredMap=new Map(desired.map(d=>[d.key,d])),existingByKey=new Map();
    for(const p of existing){const key=`${p.scheduled_for}|${p.task_type}|${p.topic_id}`;if(!existingByKey.has(key))existingByKey.set(key,[]);existingByKey.get(key).push(p);}
    let changed=false;

    for(const [key,rows] of existingByKey){
      const want=desiredMap.get(key);
      if(!want){const removable=rows.filter(r=>r.status==='pending');if(removable.length){const del=await db.from('study_plan_items').delete().eq('user_id',user.id).in('id',removable.map(r=>r.id));if(del.error)throw del.error;changed=true;}continue;}
      const keep=rows[0],extras=rows.slice(1).filter(r=>r.status==='pending');
    ...
    for(const want of desiredMap.values()){
      ...
      const ins=await db.from('study_plan_items').insert(row);if(ins.error)throw ins.error;changed=true;
    }
    return changed;
  }

  function injectStyles(){
    if($('#mentorFocusV431Css'))return;
    const style=document.createElement('style');style.id='mentorFocusV431Css';style.textContent=`
  ...
  function renderFocus(meta,m,idx){
    ...
    const focuses=(entry.subjects||[]).map(s=>({s,t:idx.focusTopic(s.id,m.today)}));
    const pair=focuses.map(x=>x.s.name).join(' + '),detail=focuses.map(x=>`${x.s.name}: ${x.t?.syllabus_code||''} ${x.t?.title||''}`).join(' • ');
    ...
    let rule=$('#focusRuleV43');if(!rule){rule=document.createElement('div');rule.id='focusRuleV43';rule.className='focus-rule-v43';document.querySelector('[data-page-view="daily"] .page-header')?.insertAdjacentElement('afterend',rule);}
    if(rule)rule.textContent=`Foco de hoje: ${detail}. Revisões do mesmo assunto são agrupadas; as dos outros assuntos ficam para a próxima passagem da matéria no ciclo.`;
  }
  ...
  async function run({reload=true}={}){
    ...
    try{
      ...
      const cleaned=await cleanupLegacyPending(m);if(cleaned)m=await loadModel();
      idx=indexes(m);const {desired,meta}=desiredPlan(m,idx,dates);const changed=await reconcileGenerated(m,desired);renderFocus(meta,m,idx);
      const key=`mentor-v431-${m.today}`;
    ...
    }catch(error){console.error('Mentor V4.3.1:',error);}finally{busy=false;}
  }
