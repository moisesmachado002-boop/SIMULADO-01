(() => {
  ...
  function setBusy(busy) {
    document.querySelectorAll('#authGate button, #accountSheet button').forEach(btn => btn.disabled = busy);
  }
  ...
  async function persistCloudState() {
    ...
    try {
      ...
      payload.cloud = { version: CLOUD_VERSION, syncedAt: nowIso() };
  ...
  async function loadOrCreateProfile(user) {
    ...
    if (cloudState && typeof state !== 'undefined') {
      applyingCloud = true;
      state = cloudState;
      originalSaveState?.();
      applyingCloud = false;
    ...
    try {
      if (mode === 'signup') {
        const { data, error } = await db.auth.signUp({
          email,
          password,
          options: { emailRedirectTo: APP_PUBLIC_URL, data: { display_name: name || email.split('@')[0] } }
        });
        if (error) throw error;
  ...
  function updateUI(user) {
    ...
    const btn = document.querySelector('#accountButton');
    if (btn) {
      btn.title = currentUser ? `Conta: ${currentUser.email || ''}` : 'Entrar e sincronizar';
      btn.querySelector('span').textContent = currentUser ? '☁' : '♙';
    }
    if (currentUser) {
      closeAuth();
      setCloudStatus('Nuvem ✓', 'ok');
  ...
  function injectUI() {
    ...
    document.body.insertAdjacentHTML('beforeend', `
      <div class="auth-gate" id="authGate" data-mode="signin">
        <div class="auth-card">
          ...
          <div class="auth-logo">M</div><span class="eyebrow">MENTOR IA • NUVEM</span>
          <h2 id="authTitle">Entrar na Mentor IA</h2>
          <p>Seu progresso e seu banco privado ficam vinculados à sua conta.</p>
          ...
          <label>Senha<input id="authPassword" type="password" autocomplete="current-password" minlength="6" placeholder="Mínimo de 6 caracteres" /></label>
          ...
          <small>As questões privadas exigem login.</small>
        </div>
      </div>
      <div class="account-sheet" id="accountSheet"><div class="account-card">
        <button class="auth-close" id="accountCloseBtn" aria-label="Fechar">×</button><span class="eyebrow">CONTA CONECTADA</span>
        <h2>Mentor IA na nuvem</h2><p id="accountEmail"></p>
        <button class="primary" id="syncNowBtn">Sincronizar agora</button><button class="secondary" id="signOutBtn">Sair da conta</button>
      </div></div>`);
    ...
    document.querySelector('#authPassword')?.addEventListener('keydown', e => { if (e.key === 'Enter') submitAuth(); });
  }

  function loadBankAssets() {
    if (!document.querySelector('#mentorBankCss')) {
      const css = document.createElement('link');
      css.id = 'mentorBankCss';
      css.rel = 'stylesheet';
      css.href = './bank-mode.css?v=1.5';
      document.head.appendChild(css);
    }
    if (!document.querySelector('#mentorBankScript')) {
      const script = document.createElement('script');
      script.id = 'mentorBankScript';
      script.src = './bank-mode.js?v=1.5';
      script.defer = true;
