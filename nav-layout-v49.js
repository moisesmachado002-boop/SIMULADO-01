(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      @media(max-width:760px){
        .v49-library-grid{grid-template-columns:1fr}.v49-filter-row{grid-template-columns:1fr}.v49-library-hero{align-items:flex-start;flex-direction:column}.v49-sub{padding-left:42px}
      }
    `;
  ...
  function addMaterialPages() {
    ...
    wrap.innerHTML = `
      <section class="v49-material-page" id="v49VideoPage" data-v49-view="videos">
        <div class="v49-library-hero">
          <div><p class="eyebrow">MATERIAIS DE ESTUDO</p><h1>Videoaulas</h1><p class="muted">Vídeos selecionados por matéria e assunto do edital.</p></div>
          <span class="v49-badge">Biblioteca preparada</span>
        </div>
        <section class="panel">
          <div class="v49-filter-row"><select disabled><option>Todas as matérias</option></select><select disabled><option>Todos os assuntos</option></select><button class="secondary-button" disabled>Filtrar</button></div>
          <div class="v49-empty"><strong>Nenhuma videoaula adicionada ainda</strong><span>Esta área já está pronta. Depois serão adicionados os links selecionados do YouTube por assunto.</span></div>
        </section>
      </section>
      <section class="v49-material-page" id="v49PdfPage" data-v49-view="pdfs">
        <div class="v49-library-hero">
          <div><p class="eyebrow">MATERIAIS DE ESTUDO</p><h1>PDFs e Arquivos</h1><p class="muted">Materiais de apoio organizados pelo edital.</p></div>
          <span class="v49-badge">Biblioteca preparada</span>
        </div>
        <section class="panel">
          <div class="v49-filter-row"><select disabled><option>Todas as matérias</option></select><select disabled><option>Todos os assuntos</option></select><button class="secondary-button" disabled>Filtrar</button></div>
          <div class="v49-empty"><strong>Nenhum PDF adicionado ainda</strong><span>Esta área já está pronta para receber PDFs, leis, resumos e outros arquivos de estudo.</span></div>
        </section>
      </section>
      <section class="v49-material-page" id="v49SimuladosPage" data-v49-view="simulados">
        <div class="v49-library-hero"><div><p class="eyebrow">QUESTÕES</p><h1>Simulados</h1><p class="muted">Área preparada para simulados completos e por matéria.</p></div><span class="v49-badge">Em preparação</span></div>
        <div class="v49-library-grid">
          <article class="v49-library-card"><div><span class="bigicon">📝</span><h3>Simulado completo</h3><p>Modelo de prova com distribuição de questões, cronômetro e relatório final.</p></div><button class="secondary-button" disabled>Em breve</button></article>
          <article class="v49-library-card"><div><span class="bigicon">📚</span><h3>Simulado por matéria</h3><p>Bateria concentrada em uma disciplina do ciclo.</p></div><button class="secondary-button" disabled>Em breve</button></article>
          <article class="v49-library-card"><div><span class="bigicon">🎯</span><h3>Simulado de fraquezas</h3><p>Questões selecionadas pela Mentora a partir dos pontos mais instáveis.</p></div><button class="secondary-button" disabled>Em breve</button></article>
        </div>
      </section>`;
  ...
  const groups = [
    {id:'questions',icon:'✓',label:'Questões',items:[
      ...
      ['Análise de Questões','page','performance'],
      ['Simulados','custom','simulados']
    ]},
    {id:'performance',icon:'◔',label:'Meu Desempenho',items:[
      ['Performance','page','performance'],
      ['Progresso do Edital','page','syllabus'],
      ['Tempo de Estudos','page','performance'],
      ['Ritmo por Matéria','page','plan'],
      ['Relatórios','page','mentor']
    ]},
    {id:'plan',icon:'⚙',label:'Meu Plano',items:[
      ['Disciplinas do Plano','page','plan'],
      ['Editar Ciclo de Matérias','page','plan'],
      ['Editar Horários','page','plan','schedule'],
      ['Replanejar Atrasos','replan'],
      ['Rever Assuntos do Plano','page','syllabus']
    ]},
    ...
    {id:'mentor',icon:'✦',label:'Mentora IA',items:[
      ['Análise Atual','page','mentor'],
      ['Alertas','page','mentor'],
      ['Recomendações','page','mentor'],
      ['Relatório para baixar','page','mentor']
    ]}
  ];
  ...
  function buildNav() {
    ...
    nav.innerHTML = `
      ...
      ${groups.map(g => `<div class="v49-group" data-v49-group="${g.id}">
        <button class="v49-parent" data-v49-toggle="${g.id}" aria-expanded="false"><span class="v49-ico">${g.icon}</span><span>${g.label}</span><span class="v49-caret">⌄</span></button>
        <div class="v49-sub" data-v49-sub="${g.id}">${g.items.map(itemButton).join('')}</div>
      </div>`).join('')}
      <div class="v49-sep"></div>
      <button class="v49-direct" data-page="syllabus"><span class="v49-ico">☷</span><span>Edital do Plano</span></button>
  ...
  function setGroupOpen(id) {
    $$('.v49-group').forEach(g => {
      const active = g.dataset.v49Group === id;
      g.querySelector('.v49-parent')?.setAttribute('aria-expanded', String(active));
      g.querySelector('.v49-sub')?.classList.toggle('open', active);
    });
  }

  function handleWrongQuestions() {
    closeMaterials();
    const target = document.querySelector('[data-page="questions"]');
    target?.click();
    setTimeout(() => {
      const bankTab = document.querySelector('[data-question-tab="bank"]');
      bankTab?.click();
      const mode = $('#bankMode');
      if (mode) { mode.value = 'review'; mode.dispatchEvent(new Event('change')); }
    }, 60);
  }
  ...
  function bind() {
    document.addEventListener('click', e => {
      ...
      if (toggle) {
        ...
        const id = toggle.dataset.v49Toggle;
        const wasOpen = toggle.getAttribute('aria-expanded') === 'true';
        if (wasOpen) {
          toggle.setAttribute('aria-expanded','false');
          document.querySelector(`[data-v49-sub="${id}"]`)?.classList.remove('open');
        } else setGroupOpen(id);
        return;
      }
      const custom = e.target.closest('[data-v49-open]');
      if (custom) {
        e.preventDefault(); e.stopPropagation();
        const v = custom.dataset.v49Open;
        if (v === 'videos') openCustom('videos','Videoaulas','Materiais de estudo por assunto');
        else if (v === 'pdfs') openCustom('pdfs','PDFs e Arquivos','Biblioteca de materiais de estudo');
        else openCustom('simulados','Simulados','Treinos completos e por matéria');
        return;
      }
      const wrong = e.target.closest('[data-v49-action="wrong"]');
      if (wrong) { e.preventDefault(); e.stopPropagation(); handleWrongQuestions(); return; }
      const normal = e.target.closest('[data-page],[data-action="replan"]');
      if (normal) closeMaterials();
    }, true);

    window.addEventListener('hashchange', () => {
      const h = location.hash.slice(1);
      if (['videos','pdfs','simulados'].includes(h)) {
        if (h === 'videos') openCustom(h,'Videoaulas','Materiais de estudo por assunto');
        if (h === 'pdfs') openCustom(h,'PDFs e Arquivos','Biblioteca de materiais de estudo');
        if (h === 'simulados') openCustom(h,'Simulados','Treinos completos e por matéria');
      }
    });
  }
  ...
  function boot() {
    ...
    bind();
    const h = location.hash.slice(1);
    if (['videos','pdfs','simulados'].includes(h)) setTimeout(() => window.dispatchEvent(new Event('hashchange')), 80);
  }

  let tries = 0;
  const timer = setInterval(() => {
    tries++;
    if ($('#appShell') && $('.sidebar-nav') && $('main.content')) { clearInterval(timer); boot(); }
