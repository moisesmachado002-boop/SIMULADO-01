(() => {
  ...
  const MINUTES_PER_QUESTION=2,MAX_REVIEW_QUESTIONS=5,TARGET_EVIDENCE=10;
  const SOURCE_PREFIX='volume_v44_';
  const OLD_PREFIXES=['focus_v43_','cycle_v4_','cycle_v41_','cycle_v42_'];
  const CYCLE=[
  ...
  async function loadModel(){
    ...
    return{today,end,prefs:prefsR.data||{study_days:[1,2,3,4,5,6]},subjects:subjectsR.data||[],topics:topicsR.data||[],mastery:masteryR.data||[],questions:questionsR.data||[],states:statesR.data||[],reviews:reviewsR.data||[],plan:planR.data||[],cycle:await ensureCycle(today)};
  }
  ...
  function indexes(m){
    ...
    const quotaFor=(topic,day,singleSubject=false)=>{
      ...
      if(singleSubject)quota=Math.max(MIN_DAILY_QUESTIONS,quota);
      return quota;
    };
    return{subjectByName,topicById,masteryByTopic,stateByQ,available,unseen,reviewsByTopic,dueReviews,maxRisk,focusTopic,quotaFor};
  }
  ...
  function desiredPlan(m,idx,dates){
    ...
    for(const day of dates){
      ...
      subjects.forEach((subject,slot)=>{
        ...
        const quota=idx.quotaFor(topic,day,single),due=idx.dueReviews(topic.id,day);
        const completedReview=m.plan.some(p=>p.scheduled_for===day&&p.topic_id===topic.id&&p.task_type==='review'&&p.status==='completed'&&String(p.source_reason||'').startsWith(SOURCE_PREFIX));
        const completedQuestions=m.plan.some(p=>p.scheduled_for===day&&p.topic_id===topic.id&&p.task_type==='questions'&&p.status==='completed'&&String(p.source_reason||'').startsWith(SOURCE_PREFIX));
        ...
        const reviewTarget=completedReview?0:existingReview?Math.max(1,Number(existingReview.question_target||1)):Math.min(MAX_REVIEW_QUESTIONS,due.length);
        const questionTarget=completedQuestions?0:existingQuestions?Math.max(1,Number(existingQuestions.question_target||1)):Math.max(0,quota-reviewTarget);
        if(reviewTarget){
          const first=due[0]||{question_id:existingReview?.question_id},risk=forgettingRisk(idx.stateByQ.get(first.question_id));
          desired.push({key:`${day}|review|${topic.id}`,scheduled_for:day,topic_id:topic.id,question_id:first.question_id||existingReview?.question_id,task_type:'review',question_target:reviewTarget,duration_minutes:reviewTarget*MINUTES_PER_QUESTION,priority:risk>=95?100:risk>=85?96:90,status:'pending',source_reason:`${SOURCE_PREFIX}review`,plan_version:PLAN,sort_order:slot*100+10,progress_count:0});
        }
        if(questionTarget){
          const useBank=(idx.unseen.get(topic.id)||0)>=questionTarget;
          desired.push({key:`${day}|questions|${topic.id}`,scheduled_for:day,topic_id:topic.id,question_id:null,task_type:'questions',question_target:questionTarget,duration_minutes:questionTarget*MINUTES_PER_QUESTION,priority:80,status:'pending',source_reason:useBank?`${SOURCE_PREFIX}bank`:`${SOURCE_PREFIX}qconcursos`,plan_version:PLAN,sort_order:slot*100+20,progress_count:0});
        }
      });
    }
    return{desired,meta};
  ...
  function injectStyles(){if($('#mentorVolumeV44Css'))return;const style=document.createElement('style');style.id='mentorVolumeV44Css';style.textContent=`
    .mentor-volume-strip{display:flex;gap:12px;align-items:center;justify-content:space-between;padding:14px 16px;margin:12px 0 18px;border-radius:14px;background:#111;color:#fff;border-left:5px solid #f2c500;box-shadow:0 8px 24px rgba(0,0,0,.08)}
    .mentor-volume-strip strong{display:block;font-size:14px}.mentor-volume-strip span{font-size:12px;opacity:.8}.mentor-volume-pair{font-weight:800;text-align:right}.volume-rule-v44{margin:10px 0 16px;padding:12px 14px;border-radius:12px;background:#fff7d6;border:1px solid #e6c959;font-size:13px;line-height:1.45}
  `;document.head.appendChild(style);}

  function renderGoal(meta,m,idx){
    const entry=meta.get(m.today)||meta.values().next().value;if(!entry)return;
    const focuses=(entry.subjects||[]).map(s=>{const locked=m.plan.find(p=>p.scheduled_for===m.today&&String(p.source_reason||'').startsWith(SOURCE_PREFIX)&&idx.topicById.get(p.topic_id)?.subject_id===s.id);const t=locked?idx.topicById.get(locked.topic_id):idx.focusTopic(s.id,m.today);return{s,t,q:t?idx.quotaFor(t,m.today,(entry.subjects||[]).length===1):0};});
    const pair=focuses.map(x=>x.s.name).join(' + '),total=focuses.reduce((s,x)=>s+x.q,0),minutes=total*MINUTES_PER_QUESTION;
    const html=`<div><strong>Ciclo ${entry.cycle.cycleNumber} • Dia ${entry.cycle.position+1} de ${CYCLE.length}</strong><span>Meta de volume: mínimo 30 questões/dia</span></div><div class="mentor-volume-pair">${esc(total)} questões • ~${esc(minutes)} min</div>`;
    for(const page of ['daily','dashboard']){const id=page==='daily'?'mentorVolumeDaily':'mentorVolumeDashboard';let host=$('#'+id);if(!host){host=document.createElement('div');host.id=id;host.className='mentor-volume-strip';const target=page==='daily'?document.querySelector('[data-page-view="daily"] .page-header'):document.querySelector('.welcome-panel');target?.insertAdjacentElement('afterend',host);}if(host)host.innerHTML=html;}
    let rule=$('#volumeRuleV44');if(!rule){rule=document.createElement('div');rule.id='volumeRuleV44';rule.className='volume-rule-v44';document.querySelector('[data-page-view="daily"] .page-header')?.insertAdjacentElement('afterend',rule);}
    if(rule)rule.textContent=`Hoje: ${focuses.map(x=>`${x.s.name} — ${x.t?.syllabus_code||''} ${x.t?.title||''}: ${x.q} questões`).join(' • ')}. Revisões contam dentro da meta da matéria; não são questões extras.`;
  }

  async function run({reload=true}={}){
    if(busy)return;busy=true;
    try{
      const ctx=await context();if(!ctx)return;injectStyles();
      let m=await loadModel(),dates=studyDates(m.today,m.prefs),cleaned=await cleanupOldPending(m);if(cleaned)m=await loadModel();
      const idx=indexes(m),{desired,meta}=desiredPlan(m,idx,dates),changed=await reconcile(m,desired);renderGoal(meta,m,idx);
      const key=`mentor-v44-${m.today}`;
      if((cleaned||changed)&&reload&&sessionStorage.getItem(key)!=='1'){sessionStorage.setItem(key,'1');setTimeout(()=>location.reload(),250);}else if(!cleaned&&!changed)sessionStorage.removeItem(key);
    }catch(error){console.error('Mentor V4.4:',error);}finally{busy=false;}
  }

  window.MentorIntelligence=Object.freeze({version:VERSION,run,forgettingRisk,cycle:CYCLE,goals:{minimumDaily:MIN_DAILY_QUESTIONS,perSubject:[BASE_PER_SUBJECT,MEDIUM_PER_SUBJECT,CRITICAL_PER_SUBJECT]}});
})();