(() => {
  'use strict';
  if (window.__mentorWeekSummaryV435) return;
  window.__mentorWeekSummaryV435 = true;
  ...
  function groupsForDay(items,ctx){
    const map=new Map();
    for(const x of items){
      ...
      if(!map.has(key))map.set(key,{topic_id:x.topic_id,subtopic_id:review?null:x.subtopic_id,subject_id:sid,policy:review?null:p,review_bundle:review,items:[]});
      map.get(key).items.push(x);
    }
    return [...map.values()].sort((a,b)=>Math.min(...a.items.map(x=>x.sort_order||0))-Math.min(...b.items.map(x=>x.sort_order||0)));
  }

  function focusHtml(g,ctx){
    const items=[...g.items].sort((a,b)=>(a.sort_order||0)-(b.sort_order||0)||String(ctx.topics.get(a.topic_id)?.title||'').localeCompare(String(ctx.topics.get(b.topic_id)?.title||''))),reviewBundle=!!g.review_bundle&&items.every(x=>x.task_type==='review'),done=items.every(x=>x.status==='completed'),mins=items.reduce((z,x)=>z+Number(x.duration_minutes||0),0);
    if(reviewBundle){
      const s=ctx.subjects.get(g.subject_id),steps=items.map(x=>`<div class="v431-step"><b>Revisar:</b> ${esc(ctx.topics.get(x.topic_id)?.title||'Assunto')} ${x.status==='completed'?'✓':''}</div>`);
      return `<article class="v431-focus review ${done?'done':''}"><div class="v431-subject">${esc(s?.name||'Estudo')} <span class="v431-badge review">REVISÕES</span></div><div class="v431-title">Revisões da matéria</div><div class="v431-steps">${steps.join('')}</div><div class="v431-total">${mins} min • ${done?'concluído':`${items.length} assunto(s)`}</div></article>`;
    }
    const t=ctx.topics.get(g.topic_id),sub=g.subtopic_id?ctx.topics.get(g.subtopic_id):null,s=t?ctx.subjects.get(t.subject_id):null,group=g.policy?.study_mode==='group',title=group?(t?.title||'Bloco conjunto'):(sub?.title||t?.title||'Atividade'),steps=[];
    ...
    return `<article class="v431-focus ${group?'group':''} ${done?'done':''}"><div class="v431-subject">${esc(s?.name||'Estudo')} ${group?'<span class="v431-badge group">BLOCO CONJUNTO</span>':sub?'<span class="v431-badge">1 POR VEZ</span>':''}</div><div class="v431-title">${esc(title)}</div><div class="v431-steps">${steps.join('')}</div><div class="v431-total">${mins} min • ${done?'concluído':'planejado'}</div></article>`;
  }
  ...
  function boot(){
    ...
    document.addEventListener('mentor-preferences-changed',e=>{const start=e.detail?.week_start||e.detail?.plan?.week_start||defaultReplanStart();activeStart=start;setTimeout(()=>refresh(start),150);});
    document.addEventListener('mentor-plan-changed',e=>{if(e.detail?.kind!=='stable_replan')setTimeout(()=>refresh(e.detail?.week_start||activeStart||inferStart()),250);});
