(() => {
  'use strict';
  if (window.__mentorWeekSummaryV431) return;
  window.__mentorWeekSummaryV431 = true;
  ...
  const today=()=>dateKey();
  let activeStart='',busy=false,lastSignature='';

  function style(){if($('#v431WeekStyle'))return;const s=document.createElement('style');s.id='v431WeekStyle';s.textContent=`
    .v431-week-note{margin:0 0 12px;padding:11px 14px;border:1px solid #d7e4ef;background:#f5f9fd;border-radius:10px;display:flex;gap:12px;justify-content:space-between;align-items:center;font-size:11px}.v431-week-note strong{font-size:12px}.v431-week-note b{background:#e3eefb;color:#244f83;border-radius:999px;padding:5px 8px;font-size:9px;white-space:nowrap}
    ...
    @media(max-width:900px){.v431-week-grid{grid-template-columns:repeat(7,220px)}}
  ...
  async function fetchWeek(start){const {data:{user}}=await db.auth.getUser();if(!user)return null;const end=addDays(start,6);
    ...
    let plan=planR.data||[];const nonReview=plan.filter(x=>x.task_type!=='review'),days=new Set(nonReview.map(x=>x.scheduled_for));const hasManual=nonReview.some(x=>String(x.source_reason||'').startsWith('manual'));const started=nonReview.some(x=>['completed','in_progress'].includes(x.status));
    if(start>=today()&&!started&&!hasManual&&days.size>0&&days.size<3){try{const {data,error}=await db.rpc('rebuild_smart_week_v431',{p_week_start:start});if(error)throw error;if(data?.status==='rebuilt'){const r=await db.from('study_plan_items').select('id,topic_id,subtopic_id,scheduled_for,task_type,question_target,progress_count,duration_minutes,status,sort_order,source_reason,completed_at').eq('user_id',user.id).gte('scheduled_for',start).lte('scheduled_for',end).neq('status','skipped').order('scheduled_for').order('sort_order');if(r.error)throw r.error;plan=r.data||[];}}catch(e){console.warn('V4.31 week auto-build',e);}}
    return {plan,topics:new Map((topicsR.data||[]).map(x=>[x.id,x])),subjects:new Map((subjectsR.data||[]).map(x=>[x.id,x])),policies:new Map((policiesR.data||[]).map(x=>[x.topic_id,x])),prefs:prefsR.data||{study_days:[1,2,3,4,5,6],daily_minutes:120,buffer_percent:15}};
  }
  ...
  function focusHtml(g,ctx){const t=ctx.topics.get(g.topic_id),sub=g.subtopic_id?ctx.topics.get(g.subtopic_id):null,s=t?ctx.subjects.get(t.subject_id):null,items=[...g.items].sort((a,b)=>(a.sort_order||0)-(b.sort_order||0));const group=g.policy?.study_mode==='group',done=items.every(x=>x.status==='completed'),review=items.every(x=>x.task_type==='review');const title=group?(t?.title||'Bloco conjunto'):(sub?.title||t?.title||'Atividade');const mins=items.reduce((z,x)=>z+Number(x.duration_minutes||0),0);const steps=[];
    ...
    const reviews=items.filter(x=>x.task_type==='review');if(reviews.length)steps.push(`<div class="v431-step"><b>Revisão:</b> ${reviews.length} item(ns)</div>`);
