(() => {
  ...
  const MAX_INTERVAL_HOURS = 1080; // 45 dias: proteção contra adiamento excessivo.
  ...
  function computeInterval(state) {
    ...
    if (!correct) {
      ...
      return {
        ...
        reason: confidence >= 5
          ? 'erro_alta_confianca'
          : wrongCount >= 2 ? 'erro_recorrente' : 'erro'
      };
    }

    let nextStage = Math.min(4, Math.max(1, currentStage + 1));
    ...
    // Depois de um histórico com erro, a primeira recuperação não salta além de 7 dias.
  ...
  async function scheduleAfterAttempt(questionId) {
    ...
    const questionResult = await db.from('questions')
      .select('id,topic_id')
      .eq('id', questionId)
      .maybeSingle();
    ...
    const updateState = await db.from('user_question_state')
      .update({
    ...
    // A tentativa atual conclui qualquer revisão pendente anterior da mesma questão.
    await db.from('reviews')
      .update({ status: 'completed', completed_at: anchor, updated_at: new Date().toISOString() })
      .eq('user_id', user.id)
      .eq('question_id', questionId)
      .eq('status', 'pending');
    ...
    if (questionResult.data?.topic_id) {
      await db.from('topic_mastery')
        .update({ next_review_at: dueAt, updated_at: new Date().toISOString() })
        .eq('user_id', user.id)
        .eq('topic_id', questionResult.data.topic_id);
    }
